...
def remove_paragraph_indents(text):
    """Удаляет абзацные отступы из текста."""
    cleaned_lines = [line.lstrip() for line in text.splitlines()]
    return '\n'.join(cleaned_lines)

# Список стоп-слов задан вручную в коде
stop_words = {
...
# Список ключевых слов для анализа (могут содержать цифры и знаки препинания)
keywords = ['علم', 'عمل', 'معرفت', 'یقین']

# Путь к папке с корпусом
folder_path = r'C:\Users\Lenovo\Documents\корпус персидской поэзии\attar'
texts = []

def clean_text(text, keep_words=None):
    """
    Очищает текст, но сохраняет указанные слова в оригинальном виде.
    keep_words - множество слов, которые не нужно очищать
    """
    if keep_words is None:
        keep_words = set()
    
    # Разбиваем текст на токены, сохраняя пробелы
    tokens = re.split(r'(\s+)', text)
    cleaned_tokens = []
    
    for token in tokens:
        if token.strip() in keep_words:
            # Оставляем ключевые слова как есть
...
for filename in os.listdir(folder_path):
    if filename.endswith('.txt'):
        with open(os.path.join(folder_path, filename), 'r', encoding='utf-8') as file:
            content = file.read()
            # Очищаем текст, но сохраняем ключевые слова
...
def filter_words(text, stop_words, keywords):
    ...
    for word in words:
        # Сохраняем ключевые слова в оригинальном виде
        if word in keywords:
            filtered_words.append(word)
        # Фильтруем остальные слова
        elif word not in stop_words and word.strip() != '':
            filtered_words.append(word)
    
    return filtered_words

# Объединяем все тексты и фильтруем слова
corpus_words = []
for text in texts:
...
def find_context_words(words_list, keywords, stop_words, window_size=40):
    """Ищет контекстные слова с учетом всех фильтров"""
    ...
    for i, word in enumerate(words_list):
        if word in keywords:
            ...
            # Фильтруем контекст, но сохраняем ключевые слова
            context = [
...
# Находим контекстные слова с учетом всех фильтров
context_words_all = find_context_words(corpus_words, keywords, stop_words)

# Подсчет частоты для каждого ключевого слова
context_results = {}
...
def create_excel_table(results, filename="collocations_table.xlsx"):
    ...
    for col, keyword in enumerate(results.keys(), start=1):
        max_len = max(len(f"{word} ({freq})") for word, freq in results[keyword].most_common()) if results[keyword].most_common() else len(keyword)
